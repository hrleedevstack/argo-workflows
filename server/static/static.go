...
func (s *FilesServer) ServerFiles(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("Content-Security-Policy", "upgrade-insecure-requests; default-src 'self' 'unsafe-inline' http://keycloak.hrleedevstack.ml:31879; connect-src 'self' http://keycloak.hrleedevstack.ml:31879; img-src 'self' data:")
	if s.hsts {
